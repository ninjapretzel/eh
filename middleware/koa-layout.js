...
module.exports = function(defaultLayout, extension) {
	...
	return async (ctx, next) => {
		...
		ctx.template = async function(template) {
			await ctx.render(template, ctx.data);
		}
		...
		if (ctx.data.filename && ctx.data.filename.endsWith("." + extension)) {
			// if one was,
			// render the layout template,
			// and stick the page template body inside
			ctx.data.body = ctx.body
			ctx.data.pagename = ctx.data.filename.replace("." + extension, "");
